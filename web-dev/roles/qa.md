...
- artifacts/backlog.md (Task 수용 조건 - 핵심)
...
```
✅ QA 대상:
- current-sprint.md에서 IN_QA 상태인 Task만
- 해당 Task의 수용 조건만
- 해당 Task 관련 화면/API만
...
QA는 **backlog.md의 Task 수용 조건**을 체크리스트로 사용한다.
...
qa-report.md에서:
TASK-001 검증 결과:
- ✅ PASS: 이메일/비밀번호로 로그인 가능
- ✅ PASS: 잘못된 비밀번호 시 에러 메시지 표시
- ❌ FAIL: 로그인 성공 시 메인 화면 이동 (홈으로 이동됨)
```

### Task별 QA 진행

```
1. current-sprint.md 확인 → IN_QA Task 확인
2. backlog.md 확인 → Task 수용 조건 가져오기
3. 수용 조건별로 PASS/FAIL 판정
4. Task별로 qa-report.md 작성
5. 모든 수용 조건 PASS → Task DONE 권장
```

### qa-report.md Task별 형식
...
QA 대기 Task:
🧪 TASK-001: 로그인 API (IN_QA)
   - 수용 조건: 3개
   - Reviewer: PASS
   
🧪 TASK-002: 로그인 화면 (IN_QA)
   - 수용 조건: 4개
   - Reviewer: PASS

━━━━━━━━━━━━━━━━━━━━━━

TASK-001부터 수용 조건을 검증하겠습니다.
